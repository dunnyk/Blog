...
class ArticleRetrieverSerializer(serializers.ModelSerializer):
    ...
    )  # Add this a new field, to handle comments. Thies will help
    ...
    def get_comments(obj):
        comments = Comment.objects.filter(
            article_id=obj.id
        )  # This object stands for the current object/instance
        serializer = CommentRetrieveSerializer(
            comments, many=True
        )  # many=true means all comments for an article are retrieved.
